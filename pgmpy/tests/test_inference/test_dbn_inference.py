...
class TestDBNInference(unittest.TestCase):
    def setUp(self):
        ...
        dbn_1.add_edges_from([(("Z", 0), ("X", 0)), (("Z", 0), ("Y", 0)), (("Z", 0), ("Z", 1))])
        ...
        cpd_trans_z_1 = TabularCPD(("Z", 1), 2, [[0.9, 0.1], [0.1, 0.9]], [("Z", 0)], [2])
        ...
        dbn_2.add_edges_from([(("Z", 0), ("X", 0)), (("X", 0), ("Y", 0)), (("Z", 0), ("Z", 1))])
    ...
    def test_forward_inf_single_variable(self):
        ...
        np_test.assert_array_almost_equal(query_result[("X", 0)].values, np.array([0.84, 0.16]))

    def test_forward_inf_multiple_variable(self):
        query_result = self.dbn_inference_1.forward_inference([("X", 0), ("Y", 0)])
        np_test.assert_array_almost_equal(query_result[("X", 0)].values, np.array([0.84, 0.16]))
        np_test.assert_array_almost_equal(query_result[("Y", 0)].values, np.array([0.6, 0.4]))
    ...
    def test_backward_inf_single_variable(self):
        ...
        np_test.assert_array_almost_equal(query_result[("Y", 0)].values, np.array([0.225, 0.775]))

    def test_backward_inf_multiple_variables(self):
        query_result = self.dbn_inference_2.backward_inference([("X", 0), ("Y", 0)])
        np_test.assert_array_almost_equal(query_result[("X", 0)].values, np.array([0.75, 0.25]))
        np_test.assert_array_almost_equal(query_result[("Y", 0)].values, np.array([0.225, 0.775]))
